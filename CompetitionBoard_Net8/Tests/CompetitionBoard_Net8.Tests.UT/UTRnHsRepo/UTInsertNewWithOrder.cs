...
{
    [TestClass]
    public class UTInsertNewWithOrder : UTRnHsRepos
    {
        [TestMethod]
        public void TestInsertNewWithOrder()
        {
            Result<RnH>? result = _rnHsRepo?.InsertNewWithOrder(3);
            Assert.IsNotNull(result);
            Assert.AreEqual(0, result.ErrorCode);
            Assert.IsNull(result.Exception);
            Assert.IsNotNull(result.Content);
            Assert.AreEqual(3, result.Content.Order);
            ...
            Assert.AreEqual(3, rnhs[7].Order);
        }
    }
}